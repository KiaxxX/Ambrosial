...
def main(user, passwd, step):
    ...
    if step == '':
        print(f"[{now}][INFO]已设置为随机步数（10000-19999）")
    ...
    result = f"[{now}][INFO]尾号为{user[-4:]}: 修改步数（{step}）" + response['message']
    print(result)
    return result
...
if __name__ == "__main__":
    print(f"[{now}][INFO]: StepS Has Started And Been Loading！It Will Take A Little Seconds To Do This, Please Keep Patient!")
    # Push Mode  # 推送选项
    # Pm = input()
    Pm = 'wx' # 微信推送
    if Pm == 'wx' or Pm == 'nwx':
        # ServerChan
        sckey = 'SCU148291Ta7250f5e9c1aee376252e50b420f9d315ff9d009203a3'
        if str(sckey) == '0':
            sckey = ''
    # elif Pm == 'tg':
    #     token = input()
    ...
    # 用户名（格式为 13800138000）
    # user = input()
    user = '17801051532#17864307818'

    # 登录密码
    passwd = 'cxp960125#Guoer961208'
    # passwd = input()
